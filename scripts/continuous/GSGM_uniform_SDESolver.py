...
class GSGM(keras.Model):
    ...
    def GaussianFourierProjection(self,scale = 30): #why?
        #half_dim = 16
    ...
    def generate(self,cond,jet_info):
        start = time.time()
        jet_info = self.EMSampler(cond,self.ema_jet,
        ...
        start = time.time()
        parts = self.EMSampler(tf.convert_to_tensor(cond,dtype=tf.float32),
                                self.ema_part,
    ...
    def EMSampler(self, cond, model,data_shape= None, jet= None, mask= None, num_steps= 200, eps=1e-5):
        batch_size = cond.shape[0]
        #t = tf.ones((batch_size,))
        t = tf.ones((batch_size,), dtype=tf.float32)
        init_x = tf.random.normal((batch_size, *data_shape), dtype=tf.float32)
        time_steps = tf.linspace(1.0, eps, num_steps)
        step_size = time_steps[0] - time_steps[1] #h/delt
        x = init_x
        self.discrete_betas = np.linspace(self.beta_0 / num_steps, self.beta_1 / num_steps, num_steps)
        self.alphas = tf.convert_to_tensor(1. - self.discrete_betas, dtype=tf.float32) #alphas
        #z = tf.random.normal(shape=tf.shape(x), mean=0.0, stddev=tf.sqrt(tf.square(g) * step_size), dtype=tf.float32)

        for istep, time_step in enumerate(time_steps):
            batch_time_step = tf.ones((batch_size, 1)) * time_step
            time_idx= num_steps- istep-1
            f, g = self.sde(batch_time_step,shape= (-1,1))
            if jet is None:
                score = model([x, batch_time_step,cond])
            else:
                score = model([x*mask, batch_time_step,jet,cond,mask])*mask
                f=tf.reshape(f,shape= (-1,1,1))
                g=tf.reshape(g,shape= (-1,1,1))
                batch_time_step = tf.reshape(batch_time_step, shape=(-1, 1, 1))
            alpha= tf.cast(tf.ones((batch_size, *data_shape), dtype=tf.float32)*self.alphas[time_idx], dtype= tf.float32)

                
            #Predictor_Method:
            mean_x = x - (f * x - (g**2) * score )*step_size #drift_coefficient_for_reverse_sde
            x = mean_x + tf.multiply(g, tf.sqrt(abs(step_size)) *tf.random.normal(shape=tf.shape(x), mean=0.0, stddev=1.0, dtype=tf.float32))#diffusion_coeff=g
            
            #Corrector step (Langevin MCMC)
            #timestep = tf.cast(t * (num_steps - 1), tf.int32)  # Equivalent to (self.sde.N - 1)
            # alpha = tf.cast(tf.gather(self.alphas, timestep), dtype=tf.float32)
            # alpha_broadcasted = tf.broadcast_to(tf.reshape(alpha, [batch_size] + [1] * len(data_shape)), [batch_size] + list(data_shape))
            #alpha = tf.gather(self.alphas, timestep)
            with tf.GradientTape() as tape:
                tape.watch(x)
                grad = score
            grad_norm = tf.cast(tf.norm(grad), dtype=tf.float32)
            noise_norm = tf.sqrt(tf.cast(tf.reduce_prod(x.shape[1:]), dtype=tf.float32))
            snr = tf.constant(0.165, dtype=tf.float32)
            langevin_step_size = 2 * (snr * noise_norm / grad_norm) ** 2* alpha
            mean_x = x + tf.multiply(langevin_step_size, grad)
            # x= mean_x + tf.multiply(tf.random.normal(shape=tf.shape(x), mean=0.0, stddev=1.0, dtype=tf.float32), tf.sqrt(langevin_step_size * 2))
            # x = x + langevin_step_size * grad + tf.sqrt(2 * langevin_step_size) * tf.random.normal(shape=tf.shape(x), mean=0.0, stddev=1.0, dtype=tf.float32)
            x_new = mean_x + langevin_step_size * grad + tf.sqrt(2 * langevin_step_size) * tf.random.normal(shape=tf.shape(x), mean=0.0, stddev=1.0, dtype=tf.float32)
            omega = 0.05  # Or some other weight between 0 and 1
            x = omega * x + (1 - omega) * x_new
